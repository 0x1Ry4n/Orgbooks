jQuery(function () {
	...
	$('.data-table').DataTable({
		...
		// // filtragem por select 
		
		// initComplete: function () {
		// 	this.api().columns([1, 2, 4]).every(function (col) { // Especifica a coluna 
		// 		let column = this;
		// 		let theadname = $('.data-table th').eq([col]).text();
		// 		let select = $('<select class="selectpicker"><option value="' + col + '">' + theadname + '</option></select>')
